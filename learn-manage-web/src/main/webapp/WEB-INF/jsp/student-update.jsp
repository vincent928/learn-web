      ...
      <div class="row">
        ...
        <div class="col-md-9 col-md-offset-3" style="margin: 50px 10px 0px;">
        	<div id="rightTable">
        		...
        		<h3><span class="glyphicon glyphicon-align-justify" style="color:#3b7e3b;"/><font style="color:green;">我的作业</font></h3>
        		</div>
        		 <div class="row" style="border-style:  solid;border-bottom: 1px;margin: 5px;border-color: #d3d3d3;height:1px;"/>
        		 <form id="newQuestion" action="/student/updateQuestion" method="post" style="height: 600px;overflow-y: auto;overflow-x:hidden; " >
        		 <input type="hidden" name="hId" id="hId" value="${hId }">
        		 <input type="hidden" name="qId" id="qId" value="${qId }">
        		 <div class="form-inline" style="margin-top: 20px;">
        		 	<div class="row">
        		 		<div class="form-group col-md-6 col-md-offset-4" style="margin-bottom: 40px;">
			      			<label>题库标题:</label>
					...
					<table id="questionList" class="form-inline">
						...
						<c:forEach items="${chooseList }" var="choose" varStatus="ch">
							<tr>
								...
								<td>
								<label>答案：</label>
									<select id="choose${ch.index }" name="choose${ch.index }"  class="form-control">
										<option value="">请选择</option>
										<option value="A" <c:if test="${choose.xzCurrent =='A'}">selected="selected"</c:if>  >A</option>
										<option value="B" <c:if test="${choose.xzCurrent =='B'}">selected="selected"</c:if> >B</option>
										<option value="C" <c:if test="${choose.xzCurrent =='C'}">selected="selected"</c:if> >C</option>
										<option value="D" <c:if test="${choose.xzCurrent =='D'}">selected="selected"</c:if> >D</option>
						...
						<tr>
							...
							<td>
								<label>答案：</label>
									<select id="judg${ju.index }" name="judg${ju.index }" value="${judg.pdCurrent }" class="form-control">
										<option value="">请选择</option>
										<option value="A" <c:if test="${judg.pdCurrent =='A'}">selected="selected"</c:if>  >A</option>
										<option value="B" <c:if test="${judg.pdCurrent =='B'}">selected="selected"</c:if>  >B</option>
									</select>
								</td>
						</tr>
						<tr>
						...
						<tr>
							<td>
								<label></label><textarea id="subj${su.index}" name="subj${su.index }" 
								 class="form-control" rows="5" cols="60">${sub.sAnswer }</textarea>
							</td>
						</tr>
						<tr><td><br></td></tr>
						</c:forEach>
